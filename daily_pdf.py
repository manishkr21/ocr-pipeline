...
import os
import shutil
from extract_pdf import download_pdf
...
# find registration number to be processsed
regno_list="select registration_no from grievance where document_id is not null order by recvd_date limit {0},10000"
regno_list=list(pd.read_sql(regno_list,con=connection)['registration_no'])
